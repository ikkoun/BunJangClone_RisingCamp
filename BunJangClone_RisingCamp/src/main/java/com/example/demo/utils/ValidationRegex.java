...
public class ValidationRegex {
    ...
    public static boolean isRegexStarRating(Double starRating) {
        String star = String.valueOf(starRating);
        String regex = "[0-5]\\.[0|5]$";
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(star);
        return matcher.find();
    }

    public static boolean isRegexRangeStatus(Integer rangeStatus) {
        String range = String.valueOf(rangeStatus);
        String regex = "^[0-3]$";
        Pattern pattern = Pattern.compile(regex);
